...
useEffect(() => {
    async function getInfo() {
        ...
        randomNumbers.map(randomNumber => {
            ...
            const genres: string[] = previews[randomNumber].genres.map(num => allGenres[num - 1].title)
                ...
                {info.map((item, index) => (
                    <Carousel.Item key={index}>
                        <div className='flex h-full'>
                            <img src={item.image} alt={`Slide ${index + 1}`} className='rounded-lg ml-36 w-1/4 h-1/4' />
                            <div className="flex flex-col pl-4 pt-2 cursor-default">
                                <h2 className="text-2xl font-semibold mb-2">{item.title}</h2>
                                <p>Seasons: {item.seasons}</p>
                                <p>Genres {item.genres}</p>
                                <p className="mb-2">Last updated: {item.updated}</p>
                                <p className="text-xs pr-36 h-40 overflow-y-hidden">{item.description}</p>
                            </div>
                        </div>
