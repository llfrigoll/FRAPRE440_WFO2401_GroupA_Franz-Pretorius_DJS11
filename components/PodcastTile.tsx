...
export default function PodcastTile({
    ...
    const genreNames = genres
        .map(
            genreId =>
                propsGenres.find(g => Number(g.id) === genreId)?.title || ''
        )
        .filter(Boolean)
    ...
    return (
        <div
            ...
            <img
                src={image}
                className="h-44 w-44 mb-5 self-center rounded-lg"
                alt={title}
            />
            <div
                data-ref="preview-content"
                className="flex flex-col px-6 max-w-full">
                <h1 className="text-slate-800 font-bold mb-2 text-nowrap truncate">
                    {title}
                </h1>
                <hr className="mb-2" />
